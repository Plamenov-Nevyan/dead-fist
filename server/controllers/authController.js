    ...
    .then((user) => {
      ...
      req.session.save((err) => {
        ...
        res.json({
          ...
          hasCreatedCharacter: user.has_created_character,
        });
      });
    })
  ...
  req.session.destroy((err) => {
    if (err) {
      return res
        .status(500)
        .json({
          message: "Failed to log out.. please try again or contact an admin!",
        });
    }
...
router.post("/get-session", (req, res) => {
  try {
    if (req.body.property === "all") {
      ...
      res.json({
        [req.body.property]: req.session.userData[req.body.property],
      });
    }
  } catch (err) {
